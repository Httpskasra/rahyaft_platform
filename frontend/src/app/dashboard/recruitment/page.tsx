...
const labels: any = {
  ...
  INITIAL_INTERVIEW: "مصاحبه اولیه",
  TECHNICAL_INTERVIEW_ASSIGNMENT: "تخصیص مصاحبه‌گر",
  TECHNICAL_INTERVIEW: "مصاحبه فنی",
  SUPERADMIN_APPROVAL: "تأیید نهایی",
  PROFILE_COMPLETION: "تکمیل پروفایل",
};
export default function Recruitment() {
  const [items, setItems] = useState<any[]>([]);
  useEffect(() => {
    recruitmentApi.applications().then(setItems);
  }, []);
  return (
    <div dir="rtl">
      ...
      <div className="overflow-hidden rounded-2xl border bg-white dark:bg-gray-900">
          ...
          <tbody>
            {items.map((a) => (
              <tr key={a.id} className="border-t">
                ...
                <td>{labels[a.currentStage] ?? a.currentStage}</td>
                <td>{a.status}</td>
                <td>
                  <Link
                    className="text-indigo-600"
                    href={`/dashboard/recruitment/${a.id}`}>
                    بررسی پرونده
                  </Link>
                </td>
              </tr>
        ...
        {items.length === 0 && (
          <div className="p-10 text-center text-gray-500">
            هنوز پرونده‌ای ثبت نشده است. ابتدا از بخش مدیریت، فرم و فرصت شغلی
            بسازید.
          </div>
        )}
      </div>

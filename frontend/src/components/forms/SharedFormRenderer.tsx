...
export function SharedFormRenderer({
  schema,
  value,
  onChange,
  disabled = false,
  ...
  value: SharedFormAnswers;
  onChange: (v: SharedFormAnswers) => void;
  disabled?: boolean;
}) {
  const set = (k: string, v: string | number | boolean) =>
    onChange({ ...value, [k]: v });
  const field = (f: SharedFormField) => {
    const base =
    ...
    const type = f.type === "rating" ? "number" : f.type;
    return (
      <input
        className={base}
        type={type}
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {s.fields.map((f) => (
              <label
                ...
                {field(f)}
                {f.helpText && (
                  <small className="text-gray-500">{f.helpText}</small>
                )}
              </label>
            ))}
          </div>

...
// نتیجه‌ی محاسبه‌شده‌ی scope که روی request قرار می‌گیرد
// تا سرویس‌ها بتوانند بدون محاسبه‌ی دوباره از آن استفاده کنند.
...
export class PermissionsGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    // Step 1: گرفتن Role و Permission کاربر (با خود Permission، نه فقط RolePermission خام)
    const userRoles = await this.prisma.userRole.findMany({
      where: { userId: user.id },
      include: {
        role: {
    ...
    const SCOPE_RANK: Record<string, number> = {
      ...
      TEAM: 1,
      DEPARTMENT: 2,
      DEPARTMENT_SUBTREE: 3,
      RELATED_DEPARTMENTS: 4,
      ORG_WIDE: 5,
    };

    const matchingPermissions = userRoles
      .flatMap((ur) => ur.role.permissions)
      .filter(
        (p) =>
          p.permission.action === requiredPermission.action &&
          p.permission.resource === requiredPermission.resource,
      );
    ...
    // بیشترین scope بین نقش‌های کاربر برای این action/resource
    const effectiveScope = matchingPermissions.reduce((widest, p) => {
      return SCOPE_RANK[p.scope] > SCOPE_RANK[widest] ? p.scope : widest;
    }, matchingPermissions[0].scope);
    ...
    // نتیجه را روی request می‌گذاریم تا سرویس‌ها بتوانند فیلتر کنند
    request.permission = {
